...
PYTHON_VIEW_HEIGHT     = PANEL_HEIGHT + 38
...
class MiiCraftModeler:
    ...
    def draw_grid(self):
        ...
        self.y_axis_arrow_0 = arrow(pos=(0, 14,  0), axis=(0, 4, 0), color=color.yellow, shaftwidth=0.2)
        self.y_axis_label_0 = label(pos=(0,17, 2), color=color.yellow, text='Y')
        self.x_axis_arrow_0 = arrow(pos=(22.5, 0,  0), axis=(4, 0, 0), color=color.yellow, shaftwidth=0.2)
        self.x_axis_label_0 = label(pos=(25.5, 0, 2), color=color.yellow, text='X')
        self.tip = label(pos=(0, 0, 0), text='Shift Z: Left-Click\nRotate: Right-Drag\nScale: (Left+Right) Drag', xoffset=-30, yoffset=-40, space=200, line=0)
    ...
    def enableShiftRotateBtn(self):
        self.label_shift_x.config(state=NORMAL)
        self.label_shift_y.config(state=NORMAL)
        self.label_shift_z.config(state=NORMAL)
        self.label_rotate_x.config(state=NORMAL)
        self.label_rotate_y.config(state=NORMAL)
        self.label_rotate_z.config(state=NORMAL)
        self.model_shift_x.config(state=NORMAL)
        self.model_shift_y.config(state=NORMAL)
        self.model_shift_z.config(state=NORMAL)
        self.model_rotate_x.config(state=NORMAL)
        self.model_rotate_y.config(state=NORMAL)
        self.model_rotate_z.config(state=NORMAL)
        self.btn_shift_x.config(state=NORMAL)
        self.btn_shift_y.config(state=NORMAL)
        self.btn_shift_z.config(state=NORMAL)
        self.btn_rotate_x.config(state=NORMAL)
        self.btn_rotate_y.config(state=NORMAL)
        self.btn_rotate_z.config(state=NORMAL)
        self.button_slicing.config(state=NORMAL)
        return
    
    def disableShiftRotateBtn(self):
        self.label_shift_x.config(state=DISABLED)
        self.label_shift_y.config(state=DISABLED)
        self.label_shift_z.config(state=DISABLED)
        self.label_rotate_x.config(state=DISABLED)
        self.label_rotate_y.config(state=DISABLED)
        self.label_rotate_z.config(state=DISABLED)
        self.model_shift_x.config(state=DISABLED)
        self.model_shift_y.config(state=DISABLED)
        self.model_shift_z.config(state=DISABLED)
        self.model_rotate_x.config(state=DISABLED)
        self.model_rotate_y.config(state=DISABLED)
        self.model_rotate_z.config(state=DISABLED)
        self.btn_shift_x.config(state=DISABLED)
        self.btn_shift_y.config(state=DISABLED)
        self.btn_shift_z.config(state=DISABLED)
        self.btn_rotate_x.config(state=DISABLED)
        self.btn_rotate_y.config(state=DISABLED)
        self.btn_rotate_z.config(state=DISABLED)
        self.button_slicing.config(state=DISABLED)
        return
    ...
    def shift_x(self, val):
        ...
        if self.model_x != 0:
            self.disableShiftRotateBtn()
            self.label_shift_x.config(state=NORMAL)
            self.model_shift_x.configure(state=NORMAL)
            self.btn_shift_x.config(state=NORMAL)
        else:
            self.enableShiftRotateBtn()
        return
    ...
    def shift_y(self, val):      
        ...
        if self.model_y != 0:
            self.disableShiftRotateBtn()
            self.label_shift_y.config(state=NORMAL)
            self.model_shift_y.configure(state=NORMAL)
            self.btn_shift_y.config(state=NORMAL)
        else:
            self.enableShiftRotateBtn()
        return
    ...
    def shift_z(self, val):
        ...
        if self.model_z != 0:
            self.disableShiftRotateBtn()
            self.label_shift_z.config(state=NORMAL)
            self.model_shift_z.configure(state=NORMAL)
            self.btn_shift_z.config(state=NORMAL)
        else:
            self.enableShiftRotateBtn()
        return
    ...
    def OpenFile(self, ask):        
        ...
        try:
            laststl = codecs.open(HOMEPATH+"\\.miicraft\\"+"laststl", "w", encoding='utf-8') 
    ...
    def load_last_stl(self):
        HOMEPATH = os.environ['HOMEDRIVE']+os.environ['HOMEPATH']
        try:
            laststl = codecs.open(HOMEPATH+"\\.miicraft\\"+"laststl", "r", encoding='utf-8') 
        except:
    ...
    def __init__(self, parent, parent_window):
        ...
        self.btn_shift_x = Button(self.ModelerFrame, text="Set", command=self.submit_xshift,font=("Helvetica", 12, 'bold'), width = 8)
        self.btn_shift_x.grid(row=3, column=3, sticky=E, padx=5,pady=YSPACING)
        ...
        self.btn_shift_y = Button(self.ModelerFrame, text="Set", command=self.submit_yshift,font=("Helvetica", 12, 'bold'), width = 8)
        self.btn_shift_y.grid(row=4, column=3, sticky=E, padx=5,pady=YSPACING)
        ...
        self.btn_shift_z = Button(self.ModelerFrame, text="Set", command=self.submit_zshift,font=("Helvetica", 12, 'bold'), width = 8)
        self.btn_shift_z.grid(row=5, column=3, sticky=E, padx=5,pady=YSPACING)
        ...
        self.btn_rotate_x = Button(self.ModelerFrame, text="Set", command=self.submit_xrotate,font=("Helvetica", 12, 'bold'), width = 8)
        self.btn_rotate_x.grid(row=6, column=3, sticky=E, padx=5,pady=YSPACING)
        ...
        self.btn_rotate_y = Button(self.ModelerFrame, text="Set", command=self.submit_yrotate,font=("Helvetica", 12, 'bold'), width = 8)
        self.btn_rotate_y.grid(row=7, column=3, sticky=E, padx=5,pady=YSPACING)
        ...
        self.btn_rotate_z = Button(self.ModelerFrame, text="Set", command=self.submit_zrotate,font=("Helvetica", 12, 'bold'), width = 8)
        self.btn_rotate_z.grid(row=8, column=3, sticky=E, padx=5,pady=YSPACING)
        ...
        """ # 20130204 Michael, We will show tip on the viewer
        # STL Viewer tips
